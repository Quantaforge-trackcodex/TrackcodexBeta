...
const CreateRepoModal: React.FC<CreateRepoModalProps> = ({ isOpen, onClose, onSubmit }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSubmit({
      ...
      techColor: formData.techStack === 'TypeScript' ? '#3178c6' : 
                 formData.techStack === 'Go' ? '#00add8' : 
      ...
      <div className="bg-[#161b22] border border-primary/30 w-full max-w-2xl rounded-3xl overflow-hidden shadow-2xl flex flex-col font-display">
        <div className="p-6 border-b border-[#30363d] bg-primary/5 flex items-center justify-between">
           <div>
              ...
              <p className="text-[10px] text-slate-500 mt-1 uppercase tracking-widest font-bold">Gitea Instance: trackcodex-cloud-01</p>
           </div>
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-6">
           <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                   <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest block mb-2 px-1">Primary Tech Stack</label>
                   <select 
                ...
                <div>
                   <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest block mb-2 px-1">Visibility</label>
           ...
           <div className="flex items-center gap-3 pt-6 border-t border-[#30363d]">
             <button type="button" onClick={onClose} className="flex-1 py-3.5 text-sm font-bold text-slate-500 hover:text-white transition-colors">Cancel</button>
             <button type="submit" className="flex-1 py-3.5 bg-primary text-white rounded-xl font-black uppercase tracking-widest text-xs shadow-lg shadow-primary/20 hover:brightness-110 transition-all active:scale-95">
                Create Repository
             </button>
           </div>
        </form>
