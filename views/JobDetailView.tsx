...
const JobDetailView = () => {
  ...
  // Combine mock jobs and any locally stored offered jobs
  ...
  const job = useMemo(() => allJobs.find(j => j.id === id), [id, allJobs]);
  ...
  if (!job) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center p-8 bg-[#0d1117] text-center">
        ...
        <h2 className="text-2xl font-black text-white mb-2">Job Not Found</h2>
        <p className="text-slate-500 mb-8">The job mission you are looking for does not exist or has been archived.</p>
        <button 
          onClick={() => navigate('/dashboard/jobs')}
          className="px-6 py-2 bg-primary text-white rounded-xl font-bold uppercase tracking-widest text-[11px]"
        >
          Back to Marketplace
        </button>
      </div>
  ...
  const handleStartWork = () => {
    // Check if job is pending or open and move to "In Progress"
    if (currentStatus === 'Open' || currentStatus === 'Pending') {
      setCurrentStatus('In Progress');
    }
    // Navigate to the repository's workspace
    navigate(`/workspace/${job.repoId}`);
  };

  const handleDecline = () => {
    if (confirm("Are you sure you want to decline this job offer?")) {
      setCurrentStatus('Open'); // Or set to a 'Declined' state if we had one
  ...
  const handleMessageClient = () => {
    directMessageBus.openChat({
      id: job.creator.name.replace(/\s+/g, '').toLowerCase(),
           ...
           <div className="flex items-center gap-6">
              ...
              <div>
                 <div className="flex items-center gap-3 mb-1">
                   <h1 className="text-2xl font-black text-white tracking-tight uppercase">{job.title}</h1>
                   <span className={`px-2 py-0.5 rounded-full border text-[9px] font-black uppercase tracking-widest bg-opacity-5 ${
                     ...
                     currentStatus === 'In Progress' ? 'border-amber-500 text-amber-500' : 
                     currentStatus === 'Pending' ? 'border-blue-500 text-blue-500 animate-pulse' :
                 ...
                 <p className="text-[11px] text-slate-500 font-mono uppercase tracking-widest">
                   {job.type} • Posted {job.postedDate} • Ref: {job.id}
                 </p>
              </div>
           </div>
             ...
             {currentStatus === 'Pending' && (
               <div className="flex gap-3">
                 <button 
                  ...
                  className="px-6 py-3 bg-transparent border border-rose-500/30 text-rose-500 hover:bg-rose-500 hover:text-white rounded-xl font-black uppercase tracking-widest text-[11px] transition-all"
                 >
                   Decline Offer
                 </button>
                 <button 
                  onClick={handleStartWork}
                  className="px-8 py-3 bg-primary text-white rounded-xl font-black uppercase tracking-widest text-[11px] transition-all shadow-lg shadow-primary/20 hover:scale-[1.02] active:scale-95"
                 >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-16">
          ...
          <div className="space-y-12">
              ...
              <div className="prose prose-invert max-w-none">
                 ...
                 {job.personalNote && (
                   <div className="mt-8 p-6 bg-primary/5 border border-primary/20 rounded-2xl italic relative overflow-hidden">
                      ...
                      <p className="text-primary text-sm font-bold uppercase tracking-widest mb-2 not-italic">Note from {job.creator.name}:</p>
                      <p className="text-slate-300 relative z-10">"{job.personalNote}"</p>
                   </div>
                 )}
              </div>
            </section>
            ...
            <section>
              ...
              <div className="bg-[#161b22] border border-[#30363d] rounded-2xl p-8 group hover:border-primary/40 transition-all cursor-pointer shadow-lg relative overflow-hidden" onClick={() => navigate(`/repo/${job.repoId}`)}>
                 ...
                 <div className="flex items-center justify-between relative z-10">
                    <div className="flex items-center gap-6">
                       ...
                       <div>
                          <p className="text-xl font-black text-white group-hover:text-primary transition-colors">{job.repoId}</p>
            ...
            {currentStatus === 'Completed' && submittedFeedback && (
              <section className="p-8 bg-emerald-500/5 border border-emerald-500/20 rounded-2xl animate-in zoom-in-95">
                 ...
                 <p className="text-xl text-slate-200 italic leading-relaxed">"{submittedFeedback}"</p>
                 <div className="mt-8 flex items-center gap-4 text-xs font-black uppercase text-slate-500 tracking-widest pt-6 border-t border-white/5">
                    <span className="flex items-center gap-2"><span className="size-2 rounded-full bg-emerald-500"></span> Reputation +15</span>
                    <span className="flex items-center gap-2"><span className="size-2 rounded-full bg-emerald-500"></span> Verified Delivery</span>
                 </div>
              </section>
            )}
          </div>
          ...
          <aside className="space-y-10">
            <div className="p-8 rounded-3xl bg-[#161b22] border border-[#30363d] shadow-2xl relative overflow-hidden ring-1 ring-white/5">
               ...
               <div className="space-y-8">
                  <div>
                     <p className="text-4xl font-black text-white tracking-tighter mb-1">{job.budget}</p>
                  ...
                  <div className="pt-8 border-t border-white/5 space-y-4">
                     <div className="flex items-center justify-between text-xs">
                        ...
                        <span className="text-white font-black">{job.type}</span>
                     </div>
                     <div className="flex items-center justify-between text-xs">
                  ...
                  <button className="w-full py-4 bg-slate-900 border border-white/10 hover:border-white/20 text-white rounded-2xl text-[11px] font-black uppercase tracking-[0.2em] transition-all shadow-xl">
                     Payment Conditions
                  </button>
               </div>
            </div>
            ...
            <div className="p-8 rounded-3xl bg-[#161b22] border border-[#30363d] shadow-xl relative ring-1 ring-white/5">
               ...
               <div className="flex items-center gap-5 mb-8">
                  <img src={job.creator.avatar} className="size-16 rounded-2xl border-2 border-primary/20 object-cover shadow-lg" alt="Client" />
                  <div>
                    <p className="text-lg font-black text-white leading-tight uppercase">{job.creator.name}</p>
            ...
            <div className="p-8 rounded-3xl border border-dashed border-[#30363d] text-center">
               <h3 className="text-[10px] font-black text-slate-600 uppercase tracking-[0.2em] mb-3">Timeline</h3>
               <div className="space-y-4">
                  <div className="flex items-center gap-4 text-left">
                     <div className="size-2 rounded-full bg-primary shadow-[0_0_8px_rgba(19,91,236,0.5)]"></div>
                     <div>
                        <p className="text-[11px] font-black text-white uppercase tracking-widest">Job Published</p>
                        <p className="text-[10px] text-slate-600">{job.postedDate}</p>
                     </div>
                  </div>
                  {currentStatus === 'Pending' && (
                    <div className="flex items-center gap-4 text-left">
                       ...
                       <div>
                          <p className="text-[11px] font-black text-blue-500 uppercase tracking-widest">Offer Extended</p>
                          <p className="text-[10px] text-slate-600">Action Required</p>
                       </div>
                    </div>
                  )}
               </div>
