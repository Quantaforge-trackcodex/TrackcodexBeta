...
const OrganizationCard = ({ org }: { org: Organization }) => {
    ...
    return (
        ...
        >
            <img src={org.avatar} className="size-16 rounded-lg border-2 border-[#30363d] p-1 object-cover" />
            <div className="flex-1">
                <h3 className="text-lg font-bold text-white group-hover:text-primary transition-colors">{org.name}</h3>
                <p className="text-sm text-slate-400 mt-1 line-clamp-1">{org.description}</p>
            </div>
            <button className="px-4 py-2 bg-[#21262d] text-slate-300 text-xs font-bold rounded-lg hover:bg-primary hover:text-white transition-all">
                Dashboard
            </button>
        </div>
    );
